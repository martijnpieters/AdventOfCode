...
inputString = """turn off 660,55 through 986,197
...
regex = re.compile(r"([a-zA-Z\s]+) (\d+\,\d+) through (\d+\,\d+)")
for s in string.split(inputString, '\n'):
    match = regex.match(s)
    if match:
        action = match.group(1)
        (top, left) = map(int, string.split(match.group(2), ','))
        (bottom, right) = map(int, string.split(match.group(3), ','))
